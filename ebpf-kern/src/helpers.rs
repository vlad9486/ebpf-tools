use core::{mem, ffi::{c_void, c_long}};
...
pub unsafe fn map_delete_elem(map: *mut c_void, key: *const c_void) -> c_long {
    let f: unsafe extern "C" fn(*mut c_void, *const c_void) -> c_long =
...
pub unsafe fn get_current_comm(buf: *mut c_void, size_of_buf: uint32_t) -> c_long {
    let f: unsafe extern "C" fn(*mut c_void, uint32_t) -> c_long =
...
pub unsafe fn ringbuf_reserve(
    ...
    let f: unsafe extern "C" fn(
        *mut c_void,
        uint64_t,
        uint64_t,
    ) -> *mut c_void = mem::transmute(131usize);
